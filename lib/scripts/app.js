...
class TfApp extends Component {
	...
	onUserChange() {
		let navigateTo = userStore.getNavigation();
		
		if(navigateTo) {			
			// Clear any navigation variables
			tfActions.clearNavigation();
			
			// Navigate the app rather than set state
			hashHistory.push(navigateTo);
			return false;
		}

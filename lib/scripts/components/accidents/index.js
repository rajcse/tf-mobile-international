		...
		<div className="multi-container">
			<section id="accidents" className="widget multi-widget premium">
				...
				{ accidents.map((accident) => (
					<div className="accident widget" key={uuid.v4()}>
						<h3 className="title">
							{ !_.isNull(accident.accident_time) ? `${accident.accident_time.day_of_week} ` : ''}
							{`${constants.months[accident.accident_date.month]}, ${accident.accident_date.day} ${accident.accident_date.year} `}
							{_.has(accident,'accident_location.city_town_name') ? `In  ${accident.accident_location.city_town_name}, ${accident.accident_state}` : '' }
						</h3>
						{ !_.isNull(accident.vehicles) ? (

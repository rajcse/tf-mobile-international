...
class CrimeModal extends Component {
	render() {
		...
		console.log(!_.isNull(offender.address) && !_.isEmpty(offender.address.city) && !_.isNull(offender.address.state_code));
					...
					{ _.map(offender.offenses, (offense, index) => {
						return (
							<div key={index}>
								...
								{ !_.isNull(offender.address) && !_.isEmpty(offender.address.city) && !_.isNull(offender.address.state_code) ?
									<div>
										<div className="label">

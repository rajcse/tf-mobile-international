...
export default class SearchForm extends Component {
	...
	handleInputChange(e) {
		...
		if(this.props.criteria.type === constants.reportTypes.PHONE) e.target.value = formatPhone(e.target.value);
	...
	doSearch(e) {
		...
		switch(this.props.criteria.type) {
			...
			case constants.reportTypes.PHONE:
				if(!this.props.criteria[constants.reportTypes.PHONE].text) return this.setState({error: 'Please enter a valid phone number'});
				search.query = {phone: this.props.criteria[constants.reportTypes.PHONE].text.replace(/[^0-9]/g, '')};
				break;
			
			case constants.reportTypes.EMAIL:
				if(!this.props.criteria[constants.reportTypes.EMAIL].text) return this.setState({error: 'Please enter a valid email'});
	...
	render() {
		...
		return (
			<form id="search-form" onSubmit={this.doSearch}>
				...
				<button type="search" onTouchTap={this.doSearch}>Search</button>
			</form>
		);
	}
	...
	renderPersonForm() {
		...
		return (
			<div id="person-search">
				...
				<select name="state" defaultValue={this.props.criteria[constants.reportTypes.PERSON].state || 'ALL'} onChange={this.handleInputChange}>
					{statesList}
				</select>
			</div>
		);

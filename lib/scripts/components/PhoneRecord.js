...
class PhoneRecord extends Component {
	...
	render() {
		...
		let reports = _.filter(record.reportData, (data) => { return !_.isEmpty(data.names[0]); });
		...
		return (
			<main>
				{ reports.map((reportData) => {
					...
					deathday = _.has(reportData,'dods[0].date') ? reportData.dods[0].date : null,
					location = !_.isEmpty(reportData.locations) ? `${reportData.locations[0].address.city}, ${reportData.locations[0].address.state_code}` : null,

...
export default class ChartBar extends Component {
  render() {
    ...
    if (!_.isEmpty(violentCrime)) {
      ...
      charts.push(
        <div className="chart-container" key={`chart-${uuid.v1()}`}>
          <h2 className="sub-title">Violent Crime Data</h2>
          <Bar data={chartData} height={75} />
        </div>
      );
    }
    ...
    if (!_.isEmpty(robbery)) {
      ...
      charts.push(
        <div className="chart-container" key={`chart-${uuid.v1()}`}>
          <h2 className="sub-title">Robbery Data</h2>
          <Bar data={chartData} height={75} />
        </div>
      );
    }
    ...
    if (!_.isEmpty(rape)) {
      ...
      charts.push(
        <div className="chart-container" key={`chart-${uuid.v1()}`}>
          <h2 className="sub-title">Forcible Rape Data</h2>
          <Bar data={chartData} height={75} />
        </div>
      );
    }

...
// import AppRate from '../../utils/AppRate';
...
export default class PubRecApp extends React.Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			reportsLookedAt: window.localStorage.getItem('reportsLookedAt'),
	...
	launchStoreRater() {
		...
		//opens up the apprate plugin modal to redirect to play store
		AppRate.preferences = {
		  displayAppName: 'TruthFinder',
		  storeAppURL: {
		  	ios: '<my_app_id>',
		  	android: 'market://details?id=com.truthfinder.app'
		  },
		  customLocale: {
		    title: "Rate TruthFinder",
		    message: "If you enjoy using TruthFinder, would you mind taking a moment to rate it? It won’t take more than a minute. Thanks for your support!",
		...
		return (
			<div>
...

				{
					...
					((this.state.reportsLookedAt == 12) && !this.state.userRated) ?
					<RatingsPrompt
						message="How are you liking our app?"
						storeRating={this.launchStoreRater}

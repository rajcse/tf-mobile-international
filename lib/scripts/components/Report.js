...
import CriminalRecordsView from './CriminalRecordsView';
...
import SexOffenderView from './SexOffenderView';
...
const PersonReport = (props) => {
	...
	return (
		<main>
			...
			<LocationView key={'location-' + Math.ceil(Math.random()*100000)} data={locations} />
			
			<CriminalRecordsView key={'criminal-' + Math.ceil(Math.random()*100000)} data={criminalRecords} />

			<SocialSectionView key={'social-' + Math.ceil(Math.random()*100000)} sources={socialProfiles} />

			<SexOffenderView key={'offenders-' + Math.ceil(Math.random()*100000)} sources={sexOffenders} />

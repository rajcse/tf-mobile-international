...
class LocalColumn extends Component {
	...
	componentWillMount() {
		...
		_.isNull(location.address.coordinates.longitude) ? this.getLocationCoordinates(location.address) : this.generateMapUrl(location.address.coordinates);
	}
...
<<<<<<< HEAD
							<button onTouchTap={ () => { openLocation(location.key); location.getLocationDetails(location); } } className="btn-link">
=======
							<button onClick={ () => { openLocation(location.key); location.getLocationDetails(); } } className="btn-link">
>>>>>>> alt/remove-touch
								Location Details
							</button>
						</div>

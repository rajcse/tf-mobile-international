...
class LocalColumn extends Component {
	...
	componentWillMount() {
		...
		_.isNull(location.address.coordinates.longitude) ? this.getLocationDetails(location.address) : this.generateMapUrl(location.address);
	}
	...
	generateMapUrl(address) {
		// Get Mapbox Image
		let coordinates = `https://api.mapbox.com/v4/mapbox.streets/${address.longitude},${address.latitude},14/300x70@2x.png?access_token=${config.MAPBOX_TOKEN}`;

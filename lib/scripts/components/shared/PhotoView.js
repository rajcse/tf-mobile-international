...
class PhotoView extends Component {
	...
	render() {
		...
		let photoGallery = this.state.photoModalOpen ?
			<div className="photo-modal" onTouchTap={() => this.closePhoto()}>
				<div className="photo-close">
					<i className="icon-close-round" onTouchTap={() => this.closePhoto()} />
				</div>
				<div className="photo-wrapper">
					<img src={config.API_ROOT + '/data/image/' + this.state.photos[this.state.currentImage].thumbnail_token} onTouchTap={this.preventClose}/>
				</div>
				<div className="photo-controller" onTouchTap={this.preventClose} >
					...
					{ photos.map((photo, key) => {
						return (
							<div key={key} onClick={() => this.openPhoto(key, photo.thumbnail_token)}>
								<img width="100" src={ config.API_ROOT + '/data/image/' + photo.thumbnail_token} />
							</div>
						);
					}) }
				</OwlCarousel>

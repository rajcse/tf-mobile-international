...
const PersonRecord = (props) => {
	...
	if (deathday) {
		...
		} else if (_.has(record.reportData,'dobs[0].age_range.low') && _.has(record.reportData,'dobs[0].age_range.high')) {
			age = record.reportData.dobs[0].age_range.low + ' - ' + record.reportData.dobs[0].age_range.high;
		}
	}

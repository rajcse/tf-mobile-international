...
class PersonRecord extends Component {
	...
	calculateDeceasedAge(date, date2) {
		// Check if dob exist
		if (_.isNull(date) || _.isEmpty(date)) {
			return null;
		}
		...
		// Format Date
		date = moment(`${date.month}/${date.day}/${date.year}`, 'MM/DD/YYYY');
		date2 = moment(`${date2.month}/${date2.day}/${date2.year}`, 'MM/DD/YYYY');
		return date2.diff(date, 'years');
	}
	...
	render() {
		...
		let birthday = _.has(record.reportData,'dobs[0].date') && !_.isNull(record.reportData.dobs[0].date)
			? record.reportData.dobs[0].date 
			: _.has(record.reportData,'dobs[0].date_range.start')
				? record.reportData.dobs[0].date_range.start
				: null;
		let deathday = _.has(record.reportData,'dods[0].date') && !_.isNull(record.reportData.dods[0].date)
			? record.reportData.dods[0].date 
			: _.has(record.reportData,'dods[0].date_range.start')
				? record.reportData.dods[0].date_range.start
				: null; 
		let age = !_.isNull(deathday)
			? record.reportData.dods[0].age
			: _.has(record.reportData,'dobs[0].age')
				? record.reportData.dobs[0].age
				: null;
		...
		return (
			<main>
				...
				{ (this.state.loading) ?
					<RecordLoader 
						reportName = {record.reportData.names[0].display}
						isPremium = {true}
					/>
					: null
				}

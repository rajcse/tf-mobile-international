...
export function calculateAge(birthday, deathday) {
	...
	if (_.isNull(birthday) || _.isEmpty(birthday)) {
		...
		if (_.isNull(reference.date)) {
			...
			reference = reference.date_range.start.year > 1900 ? reference.date_range.start : reference.date_range.end;
...
function _getFormattedDate(reference) {
	...
	reference = _.isNull(reference.date) ? reference.date_range.start : reference.date;

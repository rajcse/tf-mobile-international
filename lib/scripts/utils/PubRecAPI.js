...
class PubRecAPI {
	...
	createPhoneRecordFromNumber(number) {
		...
		.then((responseData) => {
			if(!responseData.results.length) return console.log(responseData);
	...
	purchasePackage(packageData, skipVerification) {
		...
		if(!skipVerification) {
			switch(packageData.item_type){
				...
				case 'email_report':
				//console.log(JSON.stringify(_haltedRequest));
		...
		return _makeRequest('/users/' + user.id + '/purchase-package', {needsAuth: true, method: 'POST', body: {sku: packageData.sku}})
			.then((responseData) => {
				if(responseData.success) {
					switch(packageData.item_type){
						case 'person_report': 
							this.createRecord(_haltedRequest, constants.recordTypes.PERSON);
						break;

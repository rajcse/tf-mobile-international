...
class PubRecAPI {    
    ...
    checkLocalUser() {
        ...
        if(accessToken && !_isTokenExpired(accessToken) && refreshToken && !_isTokenExpired(refreshToken)) {
            // If we have both tokens, and they aren't expired, just log the user in
            try {
            ...
            // If we have an expired access token but valid refresh, fetch a new access token then get the usage
            _refreshToken = refreshToken;
            this.refreshAccessToken().then(() => this.getUsage());
        } else {

...
class PubRecAPI {    
    ...
    search(criteria) {
		_makeRequest('/people', {query: criteria, needsAuth: true})
			.then((responseData) => {
        ...
        if(!forceRefresh) {
            ...
            if(cachedRecordIndex >= 0) {
                ...
                if(redirect) {
                    ...
                    return serverActions.redirectToRecord(cachedRecordId.recordId, reportType);
                } else {
                    // This simply announces the record was created
                    return serverActions.receiveRecordId(cachedRecordId.recordId, reportType);
                }
            }
        }

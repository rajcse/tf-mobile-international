...
function _makeRequest(path, options) {
    ...
    return fetch(config.API_ROOT + path + queryString, fetchOpts)
            ...
            .then((response) => {
                ...
                // If it's a 402, we need to throw and error to halt downstream processes
                if(response.status === 402) {
                    throw new PubRecAPIError({message: 'Payment required', statusCode: response.status});
                }
            ...
            .catch((error) => {
                ...
                // If it's a 402, call the upsell flow now
                // TODO: send the product required
                if(error.statusCode === 402) {
                    serverActions.paymentRequired();
                }

...
function _makeRequest(path, options) {
    ...
    return fetch(config.API_ROOT + path + queryString, fetchOpts)
            ...
            .catch((error) => {
                ...
                if(error.statusCode === 401) return pubRecAPI.refreshAccessToken().then(() => _makeRequest(path, options));

                // If it's a 402, call the upsell flow now, don't hit the caller's catch block
                if(error.statusCode === 402) {
                    error.responseBody.then(responseData => {
                        serverActions.paymentRequired(responseData.errors[0].item);
                    });
                }
...
class PubRecAPI {
    ...
    fetchAccountInfo() {
        ...
        return _makeRequest('/users/' + user.id, {needsAuth: true})
            .then((responseData) => {
                if(responseData.success) {
                    serverActions.receiveAccountInfo(responseData.user);
                } else {
    ...
    search(criteria) {
		return _makeRequest('/' + constants.recordEndpoints[criteria.type], {query: criteria.query, needsAuth: true})
			.then((responseData) => {
				serverActions.receiveSearchResults(responseData.results, criteria.type);
			})
            .catch((error) => {
                console.error(error);
            });
        ...
        return _makeRequest('/users/' + user.id + '/records/' + criteria.recordId, {needsAuth: true})
            .then((responseData) => {
                if(responseData.success) {
                    ...
                    serverActions.receiveRecord(responseData.record);
                } else {
                    console.log(responseData.errors);
                }
            })
    ...
    fetchLocationTeaser(pointer) {
        return _makeRequest('/' + constants.recordEndpoints[constants.recordTypes.LOCATION], {query: {pointer}, needsAuth: true})
			.then((responseData) => {
				serverActions.receiveLocationTeaser(pointer, responseData.results[0])
			})
            .catch((error) => {
                console.error(error);
            });
    ...
    purchasePremium(recordId) {
        let user = _userFromAccessToken(_accessToken);
        return _makeRequest('/users/' + user.id + '/premiumUpsell', {needsAuth: true, method: 'POST', body: recordId})

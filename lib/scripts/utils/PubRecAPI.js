...
class PubRecAPI {    
    ...
    checkLocalUser() {
        // Check there's already an _accessToken in local storage
        let token = window.localStorage.getItem('_accessToken');
        if(token && !_isTokenExpired(token)) {
            try {
                _accessToken = token;
                serverActions.receiveUser(_userFromAccessToken(token));
            } catch(e) {
        ...
        _makeRequest('/login', {method: 'POST', body: credentials})
            .then((responseData) => {
                if(responseData.success) {
                    ...
                    window.localStorage.setItem('_accessToken', _accessToken);
                    window.localStorage.setItem('_refreshToken', _refreshToken);

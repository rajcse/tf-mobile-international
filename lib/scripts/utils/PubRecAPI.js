...
class PubRecAPI {
    ...
    createRecord(recordData, reportType) {
        fetch(config.API_ROOT + '/person', {
                headers: {
    ...
    // TODO: Make this switch on report type
    // Use this to create a record
    createRecordAndRedirect(recordData, reportType) {
        fetch(config.API_ROOT + '/person', {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + _jwt
    ...
    fetchReport(criteria) {
		fetch(config.API_ROOT + '/people/' + criteria.recordId, {
                'Authorization': 'Bearer ' + _jwt
            })
            .then((response) => {
                // If it's not JSON, throw an error before it just bellyflops trying to parse it
    ...
    getUsage(userId) {
        fetch(config.API_ROOT + '/users/' + userId + '/usage', {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'Authorization': 'Bearer ' + _jwt
                }

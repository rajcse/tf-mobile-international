// TODO: Centralize the fetch method and error handling
...
/**
 * Quick 'n dirty SO find for serializing objects into a query string
...
class PubRecAPI {    
    ...
    checkLocalUser() {
        ...
        if(accessToken && !_isTokenExpired(accessToken) && refreshToken && !_isTokenExpired(refreshToken)) {
            // If we have both totkens, and they aren't expired, just log the user in
            try {
        ...
        } else if(accessToken && refreshToken && !_isTokenExpired(refreshToken)) {
            // If we have an expired access token but valid refresh, fetch a new access token
            _refreshToken = refreshToken;
    ...
    refreshAccessToken() {
        _makeRequest('/refresh-access-token', {method: 'POST', body: {refreshToken: _refreshToken}})
            .then((responseData) => {
                if(responseData.success) {
    ...
    login(credentials) {        
        ...
        _makeRequest('/login', {method: 'POST', body: credentials})
            .then((responseData) => {
                if(responseData.success) {
                    // Set the access token for future calls
                    _accessToken = responseData.accessToken;
    ...
    fetchReport(criteria, forceRefresh) {
        ...
        _makeRequest('/people/' + criteria.recordId, {needsAuth: true})
            .then((responseData) => {
                if(responseData.success) {
    ...
    getUsage(userId) {
        _makeRequest('/users/' + userId + '/usage', {needsAuth: true})
            .then((responseData) => {
                if(responseData.success) {

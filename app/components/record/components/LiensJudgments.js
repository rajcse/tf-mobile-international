			...
			{ liensJudgments.map((lien, index) => (
				<div className="document lien-individual" key={index}>
					<div className="label label-full">
						<h3 className="document-title">{getFileDate(lien.origin_filing_date)}</h3>
					</div>
					...
					{(lien.amount) ?
						<SimpleRow
							key={`amount-${uuid.v4()}`}
							content={libs._getFormattedCurrency(lien.amount)}
							title="Amount"
						/> : null
					}
					{(lien.legal_lot) ?
...
LiensJudgments.propTypes = {
	liensJudgments: React.PropTypes.array.isRequired
};

export default LiensJudgments;
